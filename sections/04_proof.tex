...
\begin{frame}{Proof of Lemma~\ref{lem:dom_pairs}}
    ...
    \uncover<1->{
        ...
        \[
            \mathbb{P}_{red} \leadsto \mathbb{P}_{red, left}, \mathbb{P}_{red, right} \qquad \mathbb{P}_{blue} \leadsto \mathbb{P}_{blue, left}, \mathbb{P}_{blue, right}
        \]
    }
    \uncover<2->{
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}[scale=0.6, auto, swap]
            ...
            \only<3>{
                ...
                \node[redpointalpha] (b) at (4.5, 3.5) {};
                \node[barlabel] (labelleft) at (1.25, 5.75) {$\mathbb{P}_{red, left}$};
                \node[fill=red, fill opacity=.1, fit=(bound1) (middlehook1)] (redleft) {};
            }
            
            % red right plotting
            \only<4>{
                \node[redpointalpha] (a) at (1, 1) {};
                \node[redpoint] (b) at (4.5, 3.5) {};
                \node[barlabel] (labelright) at (4.75, 5.75) {$\mathbb{P}_{red, right}$};
                \node[fill=red, fill opacity=.1, fit=(middlehook2) (bound2)] (redright) {};
            }
            ...
            \only<5>{
                ...
                \node[bluepointalpha] (C) at (5, 5) {};
                \node[barlabel] (labelright) at (1.25, 5.75) {$\mathbb{P}_{blue, left}$};
                \node[fill=blue, fill opacity=.1, fit=(bound1) (middlehook1)] (blueleft) {};
            }

            % blue right plotting
            \only<6>{
                \node[bluepointalpha] (A) at (3, 0.5) {};
                \node[bluepointalpha] (B) at (2, 3) {};
                \node[bluepoint] (C) at (5, 5) {};
                \node[barlabel] (labelright) at (4.75, 5.75) {$\mathbb{P}_{blue, right}$};
                \node[fill=blue, fill opacity=.1, fit=(middlehook2) (bound2)] (blueright) {};
            }
...
\begin{frame}{Proof of Lemma~\ref{lem:dom_pairs}}
    ...
    \uncover<3->{
        We do not consider $\mathbb{P}_{red, right} \cup \mathbb{P}_{blue, left}$, because $\forall p \in \mathbb{P}_{red, right}, q \in \mathbb{P}_{blue, left}: q_d \leq m_d < p_d$.
...
\begin{frame}{Proof of Lemma~\ref{lem:dom_pairs}}
    \[
        T_d(n) \leq 2 T_d\left( \frac{n}{2} \right) + {\color{Red}T_{d - 1}(n)} + \mathcal{O}(n)?
    \]
    ...
    \uncover<2->{
        \begin{figure}
            ...
            \begin{tikzpicture}[scale=0.6, auto, swap]
                ...
                % splitter, alpha highlights and set labels
                \uncover<3->{
                    \node[ghost] (hook1) at (3, -.5) {};
                    \node[ghost] (hook2) at (3, 6.5) {};
                    \path[divides] (hook1) -- (hook2);
...
\begin{frame}{Proof of Lemma~\ref{lem:dom_pairs}}
    ...
    \uncover<2->{
        $\leadsto$ Output: $n^2$ pairs in the worst case \uncover<3->{$\implies T_0(n) = \mathcal{O}\left( n^2 \right)$ \Lightning{}}
    }

    \uncover<4->{
        $\leadsto$ In the recurrence relation we're completely ignoring the output cost the pairs. \uncover<5>{$\implies \mathcal{O}(k)$}
    }
\end{frame}
...
\begin{frame}{Proof of Lemma~\ref{lem:sub_mat_mul}}
    \begin{exampleblock}{Note}
        ...
        For $k \neq \tilde{k}$ we need to break ties such as $a_{i, k} + b_{k, j} \leq a_{i, \tilde{k}} + b_{\tilde{k}, j}$ where ``$\leq$'' is fulfilled by ``$=$'' to obtain a uniquely defined minimum.
        W are considered to be ``$<$'' if $k < \tilde{k}$.
    \end{exampleblock}
\end{frame}
...
\begin{frame}{Proof of Lemma~\ref{lem:sub_mat_mul}}
    ...
    \uncover<3->{
        Because the set $\{ 1, \dots, d \}$ is finite we can choose $\hat{k} := \argmin\limits_{m = 1, \dots, d} a_{i, m} + b_{m, j}$.
    }
...
\begin{frame}{Proof of Theorem~\ref{thm:mat_mul}}
    We split our matrices $A$ and $B$ into $\begin{aligned}\frac{n}{d}\end{aligned}$ blocks, that is $\begin{aligned}\forall m = 1, \dots, \frac{n}{d}: A_m \in \mathbb{R}^{n \times d}, B_m \in \mathbb{R}^{d \times n}\end{aligned}$ for some fixed $d$.
    (If necessary we round $\begin{aligned}\frac{n}{d}\end{aligned}$ and adjust the number of blocks accordingly.)

    \uncover<2->{
        We then compute the distance products $A_i \otimes B_i$ for all $\begin{aligned}i = 1, \dots, \frac{n}{d}\end{aligned}$, and set the product to be defined by the element-wise minimum, i.e.\ $\begin{aligned}[t]c_{i, j} := \min\limits_{m = 1, \dots, \frac{n}{d}} {\left( A_m \otimes B_m \right)}_{i, j}\end{aligned}$, where $i, j = 1, \dots, n$.
    }
...
\begin{frame}{Proof of Theorem~\ref{thm:mat_mul}}
    ...
    \uncover<2->{
        ...
        \[
            c_\varepsilon^d = c_\varepsilon^{n^{1 - \varepsilon}} \sim \mathfrak{c}^n \text{ \Lightning{}}
        \]
    }
    ...
    \uncover<4->{
        $\implies \mathcal{O}\left( c_\varepsilon^d n^{2 + \varepsilon} + \frac{n^3}{d} \right) = \mathcal{O}\left( \frac{n^3}{\log(n)} \right)$. \qed{}
    }
\end{frame}
