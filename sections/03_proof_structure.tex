...
\begin{frame}{Basic Ideas}
    ...
    \begin{enumerate}
        \item<1-> A problem in computational geometry\uncover<4->{$\implies$ \emph{Finding dominating pairs}}
        \item<2-> Computing products of matrices\uncover<5->{$\implies$ \emph{Sorting indices in sets}}
        \item<3-> Combining the two ideas\uncover<6->{$\implies$ \emph{Multiplying matrices efficiently}}
    \end{enumerate}
\end{frame}
...
\begin{frame}{Splitting Quadratic Matrices}
    ...
    \uncover<2->{\textbf{A:} Split and multiply individually!\only<2->{ (Strassen's Method\footnote[2]{\cite[Section~6.2]{Aho1974}})}}
...
\begin{frame}{Multipyling Matrices and Computing their Closure\footnote{\cite[Section~5.6--5.9]{Aho1974}}}
    \begin{alertblock}{Problem}
        Strassen's method does not work for closed semirings.\footnote[1]{\cite{Chan2007}}
        However we want to work with the min-plus product which induces a closed semiring.\footnote{\cite[Example~5.9]{Aho1974}}
    \end{alertblock}
    ...
    \uncover<3->{
        ...
        \begin{theorem}
            Under some conditions on the computational time needed to multiply two square matrices and the time needed to compute the closure of a square matrix, both times are of the same order.
        \end{theorem}
    }

    \uncover<4>{
        This can then be related to the APSP problem via the closure under the min-plus product.\only<4>{\footnote{\cite[Section~5.9, Corollary~2]{Aho1974}}}
    }
\end{frame}
