...
\begin{frame}{Recovery of Shortest Paths}
    We want to recover the shortest path from vertex $I$ to vertex $j$.
    Consider $(i, j) \in X_k$.

    \uncover<2->{
        This means that the shortest path from $i$ to $j$ must go through $k$ because: $\forall k' = 1, \dots, n: w_{i, k} + w_{k, j} \leq w_{i, k'} + w_{k', j}$.

        (\emph{Taking a path through any vertex other than $k$ increases the weight.})
    }

    \uncover<3->{
        For neighbouring vertices, where the shortest path is the edge directly between them, we get that $(i, j) \in X_l$, with $l = \min \{ i, j \}$ because (assuming $i < j$): $\forall k' = 1, \dots, n: w_{i, i} + w_{i, j} \leq w_{i, k'} + w_{k', j}$.

        (\emph{Direct shortest paths between neighbouring vertices $i$ and $j$ fall into $X_i$ or $X_j$.})
    }
\end{frame}

\begin{frame}{Recovery of the Shortest Path}
    \begin{algorithm}[H]
        ...
        \myproc{get\_shortest\_path($i, j$)}{
            ...
            \Else{
                Return $\mathfrak{p} := (i, j)$\;
            }
        }
    \end{algorithm}

    With the assumption that $(a, \dots, b) \oplus (b, \dots, c) = (a, \dots, b, \dots, c)$.
\end{frame}
